...
def getBooksCasaLibro(query, bookLimit, ebook):
...
def getBooksLibCentral(query, bookLimit, ebook):
    books = []
    bookDf = pd.DataFrame()
...
def getBooksIberLibro(query, bookLimit, ebook):
    bookDf = pd.DataFrame()
    books = []
...
def getBooksAmazon(query, bookLimit, ebook):
    books = []
    bookDf = pd.DataFrame()
    options.add_argument("-private")
    driver = webdriver.Firefox(service=service, options=options)
        ...
        try:
            ...
            for i in range(len(price_List)):
                try:
                    ...
                    if ("Kindle" in detail or "Audiolibro" in detail) and ebook:
                        continue
